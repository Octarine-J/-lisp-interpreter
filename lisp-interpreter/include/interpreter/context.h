...
private:
    ...
    template <typename V>
    [[nodiscard]] std::optional<V> find_in_context(const std::map<std::string, V> &ctx, const std::string &name) const {
...
public:
    ...
    void register_function(const std::string &name, const LispFunction& function) {
        functions.insert(std::make_pair(name, function));
    }

    [[nodiscard]] std::optional<EvaluatedExpression> find_variable(const std::string &name) const {
